                                    ...
                                    const tripoint &submap )
{
    ...
    for( locx = 0; locx < SEEX; locx++ ) {
        for( locy = 0; locy < SEEY; locy++ ) {
            ...
            field &curfield = current_submap->get_field( map_tile.pos_ );
            ...
            const tripoint p = tripoint( map_tile.pos_ + sm_offset, submap.z );
...
static void field_processor_fd_push_items( const tripoint &p, field_entry &, field_proc_data &pd )
{
    ...
    for( auto pushee = items.begin(); pushee != items.end(); ) {
        if( pushee->typeId() != itype_rock ||
            ...
            for( const tripoint &dst : points_in_radius( p, 1 ) ) {
                if( dst != p && pd.here.get_field( dst, field_fd_push_items ) ) {
                    valid.push_back( dst );
                }
            }
            if( !valid.empty() ) {
    ...
    if( cur.get_field_intensity() > 1 ) {
        ...
        for( int i = 0; i < num_bolts; i++ ) {
            ...
            for( int n = 0; n < dist; n++ ) {
                bolt += dir;
                pd.here.add_field( tripoint( bolt, p.z ), field_fd_electricity, rng( 2, 3 ) );
                if( one_in( 4 ) ) {
                    if( dir.x == 0 ) {
                        dir.x = rng( 0, 1 ) * 2 - 1;
                    } else {
