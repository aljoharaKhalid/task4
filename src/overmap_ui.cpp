    ...
    if( pmenu.ret >= 0 ) {
        ...
        ui.on_redraw( [&]( const ui_adaptor & ) {
            ...
            mvwprintz( w_editor, point( 1, 3 ), c_light_gray, _( "Rotation: %s %s" ), rotation,
                       can_rotate ? "" : _( "(fixed)" ) );
            wattron( w_editor, c_red );
            mvwprintw( w_editor, point( 1, 5 ), _( "Highlighted regions" ) );
            mvwprintw( w_editor, point( 1, 6 ), _( "already have map content" ) );
            // NOLINTNEXTLINE(cata-text-style): single space after period for compactness
            mvwprintw( w_editor, point( 1, 7 ), _( "generated. Their overmap" ) );
            mvwprintw( w_editor, point( 1, 8 ), _( "id will change, but not" ) );
            mvwprintw( w_editor, point( 1, 9 ), _( "their contents." ) );
            wattroff( w_editor, c_red );
